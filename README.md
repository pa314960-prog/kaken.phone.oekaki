...
- **書き込み**: 新規作成(create)のみ許可し、以下を検証する
  ...
  - 内容の更新(update)は禁止
- **削除**: 誰でも可。`gallery.html` の各投稿に表示される削除ボタン(×)から、不適切な投稿などを取り除けるようにするため
...
削除機能についても同様に認証の仕組みがないため、Firestoreの内容(コレクション名・ドキュメントID)を知っていれば、`gallery.html` を開かなくてもAPI経由で誰でも削除できてしまいます。削除ボタンはあくまで「誤って残った投稿を運営側が消せるようにするための機能」であり、悪意のある第三者からの削除を防ぐものではない点にご注意ください。
